var Expenses_Datatable= {
    init:function() {
        var t;
        var expenses_data = $("#expenses_datatableurl").val();
        var edit_url = $("#expenses_edit_url").val();
        var delete_url = $("#expenses_delete_url").val();
        t=$(".expenses_table").mDatatable( {
            data: {
                type:"remote", source: {
                    read: {
                        url:expenses_data, map:function(t) {
                            var e=t;
            ...
            , {
                field:"expenses_date", title:"Expenses Date", width: 80, template:function(u) {
                    if(u.expenses_date=="0000-00-00" || u.expenses_date==null){
                        return '-'
                    }else{
                        return u.expenses_date
                    } 
                }
            }
            , {
...
function delete_record(){
    ...
    $.ajax({
        ...
        success: function (data){
            ...
            $('#expenses_id_'+id).closest('tr').remove();
        }
    });
}
